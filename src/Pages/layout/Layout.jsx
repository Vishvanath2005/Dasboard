...
const Layout = () => {
  ...
  return (
    <div>
      <div className="flex">
        <div    className={`${
          !open ? ` md:w-24 sm:relative  w-1/5  sm:w-1/6` : `sm:w-4/6  w-3/5  sm:static absolute   md:w-1/6 `
        }  h-screen border-r-2 drop-shadow-lg bg-white duration-300`}>
          <div className={` ${!open? `pt-8 pb-6 `:` pl-5 pt-8 pb-8` }`}>
            <img src={Logo} alt="Logo" />
          </div>
          <div className={` ${!open?`static flex justify-center  hover:bg-slate-200  `:`absolute bg-orange rounded-full sm:left-44 rotate-180 left-[327px]`} top-16  duration-500   p-2`} onClick={()=>{setOpen(!open)}}>
            <TbLayoutSidebarRightCollapse className={`${!open?`text-black`:`text-white`} text-2xl `} />
          </div>
          <ul >
          {Menus.map((menu, index) => (
            <React.Fragment key={index}>
              <NavLink to={menu.to}>
            <li
             className={`${!open?`justify-center`:`pl-10`} cursor-pointer text-md flex items-center  text-gray-500 font-medium font-Source Sans Pro gap-x-3 p-3 mt-1  transition-all duration-700 hover:bg-blue-100 hover:text-blue-500  ${
